...
그리고 Integer.MAX_VALUE로 초기화 해놓는 이유는 한 지점에서 한 지점까지의 직접적인 경로가 연결되어 있지 않아서 , 아직 해당 지점까지 가는 값을 모르기 때문에
첫 방문하는 값이 아무리커도 일단은 해당 경로에 방문했다라는 표시를 하기 위해서 Infinite 로 초기화 해놓는 것이다.
 */
public class Main2 {
