...
public class Main {
    ...
    public static int dfs(int left , int right , int index){
        ...
        /*
        해당 문제는 Top - Down 방식으로 푼 것이다 , 일단 끝까지 진행을 쭈욱
        한다음에 , 끝 점에서 이제 받은 기준을 가지고 , 차츰차츰 풀어나가는 것이다
        이 경우 작은 예제로 보자면
        1 2 2 4 의 경우를 봐보자
        일단 1 -> 2 -> 2 -> 4 이렇게 간다
        2 -> 4 로 가는 경우는 무조건 지금 왼쪽발을 기준으로 먼저 진행하였기 때문에
        왼쪽발이 1 -> 2 -> 2 -> 4 로 간 상황이다.
        그러면 끝점에 도달하였을 때에 dp[4][0][4] 인 상황이다.
        근데 이것의 값은 -1 이다 , 끝점에서는 dp 에다가 집어넣지 않기 때문이다 , 굳이 방문처리를 할 필요도 없다.
        그래서 dp[4][0][4] 에 도달하여서 0을 반환하고 , dp[2][0][3] 의 경우로 돌아오면 현재 0 + move(2 , 4) 이기 때문에 ,0 + 4이다.
        이제 여기서 오른쪽 발을 움직이는 것을 시도한다 dp[2][4][4] 도달했다가 , 다시 돌아와서 0 + move(0 , 4) 가 실행된다.
        그러면 2 가 더해진다.
        2이다 이것은 작은 값이 2이기 때문에 dp[2][0][3] = 2 가 채워지게 된다.
        지금은 왼쪽 발이 움직인 상황으로만 쭉 갔기 때문에 , dp[2][0][3] 이 상황에서 분기하여서 상황이 되게 단조롭다
        하지만 , 이제 점점 더 밀려오는 이전 재귀문들의 호출들은 더 많은 dp를 채워줄 것이다
        이 때 , 분명히 반복되는 것이 엄청나게 많을 것이다 , 각 상황에서만 하더라도 2가지의 경우로 분기 되기 때문에
        이론상으로는 2 ^ n 이기 때문이다.
        하지만 dp 로 많은 상황들을 제거해 주는 것이다.

        이렇게 해서 답을 구할 수 있다 ,
        dp[0][0][0] 값을 답으로 구하는 것 이전에 풀었던 내리막길과 비슷한 형식의 문제 Top - Down 문제인 Dance Dance Revolution 이였다.
         */
    }
