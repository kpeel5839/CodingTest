 ...
 * -- 해결
 * 드디어 맞췄다........
 * 진짜 코드 다 짜놓고서 2시간에서 3시간정도 삽질한 것 같다.
 *
 * 원래 짠 코드에 문제점은 너어어어어무우우우 많았다.
 *
 * 문제점이였던 것을 정리해보자.
 *
 * 웬만한 메소드는 다 정확하게 작성했었다.
 * 근데 복붙하는 과정에서 내가 미친짓을 한 것이
 * crossPoint 메소드이다.
 *
 * p1Dir 이 == 'y' 이면 y 축으로 이동하는 것이라고 생각하고 짰었어야 했는데
 * 그냥 코드짜기 귀찮아서 내가 짠 코드 무지성 복붙 하다가, 진짜 개해맸음
 *
 * 그리고 또 하나 문제 풀면서 애용했던 ABCD 이론에서
 * 본인이 A C D B 같은 상황은 절대 없다고 했었는데
 * 무슨 개뿔 있었다.
 *
 * 방법은 A <-> B 선분이 겁나게 길면 된다.
 * 그러면 C D 선분을 먹을 수 있다.
 * 당연히 게임 내에서는 이전에 끝나버리니까, 약간 이상하게 상상했었던 것 같다.
 *
 * 그래서 이렇게 바꿨다.
 * p1 은 무조건 현재 선분이고, p2 는 비교하는 선분이라고 가정하고 p1 의 방향성을 체크하여서
 * 먼저 닿는 것이 어떤 지점인지 판단하였다.
 *
 * 그렇게 했더니 맞았다.
 *
 * 그리고 또 head 에 대한 처리였다.
 * 이 부분도 약간 많이 까다로운 점들이 있었다.
 *
 * head 를 실질적으로 한칸 더 움직이자니 이동하는 칸 수가 이상해져 버리고 그래서
 * head 는 그대로 내버려두고 pathStart 라는 변수를 만들어서 선분의 시작점을 정의 했다.
 * 하지만, 반례는 항상 존재하는 법 (0, 0) 을 포함하지 못해서 교차판정이 안되는 문제점이 있었고
 * 그렇기에, head.y == 0 && head.x == 0 인 경우는 그냥 pathStart = new Point(0, 0) 으로 설정하는 무식한 방법을 하였다.
 * 하지만 이 때는 이 생각밖에 떠오르지 않았다.
 *
 * 그래서, 이런 식으로 진행하였고 교차판정 및 crossPoint 에서도 교차하는 경우, 즉 십자가 모양이 나오는 경우에서의 crossPoint 는 매우 잘 뽑아냈다.
 * 당연한 결과 였다.
 *
 * 그리고 마지막으로 벽과 부딪히는 계산을 먼저 하면 안된다라는 것이였다.
 * 왜냐? 벽에 부딪히는 것은 최종 생각해야 하는 부분인거고, 벽에 부딪히기 전에 선분에 먼저 부딪히기 때문이였다.
 *
 * 그래서, 선분에 부딪히는 경우를 먼저 계산하는 것과
 * 그리고 무지성 모든 선분과 교차 판정을 하면 안됬엇다.
 *
 * 예를 들어서 y 축으로 그냥 선들을 개 많이 그어놓은다음에
 * x 축으로 길게 선을 하나 쭉 그으면 교차하는 선들이 너무 많기 때문이다.
 *
 * 그래서 boolean gameOver 라는 변수를 두어서, 게임이 끝난 것만 체크하고
 * 선분은 무조건 다 검사하며 거기서 가장 최솟값, 즉 가장 먼저 닿게 되는 선분만을 취했다.
 *
 * 그래서 이 부분들을 모두 고치니 정답처리를 받을 수 있었다.
 *
 * 실수한 점을 정리하면
 * CrossPoint 계산
 * head 처리 실수 (원점에 대한 처리가 안되어서 선분으로 (0, 0)이 포함 안되는 문제가 있었음)
 * 충돌 처리 실수 (벽에 부딪히는 것보다 다른 선분과의 충돌을 먼저 체크했었어야함)
 * 마지막으로 가장 나와 먼저 부딪히는 선분에 대한, 즉 뱀 게임 측면에서 생각한 충돌처리 (snake 를 전부 돌면서 모두 체크해본다음 거기서 가장 작은 값을 취했어야 했음)
 */
