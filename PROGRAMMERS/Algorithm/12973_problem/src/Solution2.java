...
class Solution2 {
    public int solution(String s) {
        ...
        while (true) {
            if (index == sb.length() - 1) { // 현재 pointer 가 sb.length - 1 을 가르키고 있다면, 즉 가장 마지막이라면
                break;
            }

            if (sb.length() == 0) { // 아얘 끝난 경우도 레츠 고도리맨
                break;
            }
            ...
            if (a == b) { // 두개가 일치한 경우                                    
                sb.delete(index, index + 2); // 구간 삭제해주고
