 ...
 *
 * 일단, 절대적으로 길이가 긴 것들은, 남의 접두사가 되지 않고, 결론적으로 이 단어를 선택하더라도, 본인 때문에, 남에게 피해를 주지는 않는다.
 * 다만, 나의 일부를 접두사로 가지고 있는 애들이, 들어오지 못하지만 걔내는, 원래 걔내가 피해를 주는 것이라고 생각하면 된다.
 *
 * 그래서, 잘 생각해보면, 나로 인해서 못들어가는 것들이 많다면? 걔내들은 짜피 지들중에 하나라도 못들어가면 다 못들어간다.
 * 그렇기 떄문에, 길이가 긴 것을 우선적으로 넣는 것이 연산적으로도 편하다라는 것을 알 수가 있다.
 */
public class Main {
