...
public class Main {
    ...
    public static boolean check(Point before , Point after , int heaterDir , int dir){
         ...
         */
        if(dir % 2 == 0){
            if(before.x == after.x && wallY[Math.min(after.y , before.y)][before.x] == 1) return false;
            if(before.y == after.y && wallX[before.y][Math.min(before.x, after.x)] == 1) return false;
        }else{
            int firstDir;
            int secondDir = heaterDir;
//            System.out.println("heaterDir : " + heaterDir + " dir : " + dir);
            if((heaterDir - 1 < 0 ? 7 : heaterDir - 1) == dir){
                firstDir = dir - 1 < 0 ? 7 : dir - 1;
            }else{
                firstDir = dir + 1 == 8 ? 0 : dir + 1;
            }
//            System.out.println("firstDir : " + firstDir + " secondDir : " +secondDir);
            after = new Point(before.y + dy[firstDir] , before.x + dx[firstDir]);
//            System.out.println("first Move dir : " + firstDir + "  (" + before.y + "," + before.x + ") -> (" + after.y + "," + after.x + ")");
            boolean first = check(before , after , heaterDir , firstDir);
            before = after;
            after = new Point(before.y + dy[secondDir] , before.x + dx[secondDir]);
//            System.out.println("second Move dir : " + secondDir + "  (" + before.y + "," + before.x + ") -> (" + after.y + "," + after.x + ")");
...
//            System.out.println(startDir);
            ...
            while(!queue.isEmpty()){
                ...
                for(int j = startDir; j != ((dir + 1 == 8 ? 0 : dir + 1) + 1 == 8 ? 0 : dir + 2); j = (j + 1 == 8 ? 0 : j + 1)){
                    int ny = point.y + dy[j];
                    int nx = point.x + dx[j];
                    if(ny < 0 || ny >= r || nx < 0 || nx >= c || visited[ny][nx] == 1 || !check(new Point(point.y , point.x) , new Point(ny , nx) , dir , j)){
                        continue;
                    }
                    visited[ny][nx] = 1;
                    map[ny][nx] += value - 1;
...
//        for(int v = 0; v < 2; v++) { //wallX , wallY 출력
//            int[][] print;
//            int endR = 0 , endC = 0;
//            if(v == 0) {print = wallX; System.out.println("wallX"); endR = r; endC = c - 1;}
//            else {print = wallY; System.out.println("wallY"); endR = r - 1; endC = c;}
