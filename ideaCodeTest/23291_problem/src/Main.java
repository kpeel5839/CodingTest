...
4. 그냥 2개 이상 쌓여있는 것들 높이 height 변수로 증위연산자 사용해서 재고
5. 그리고 2개 이상 쌓여있는 거 어디에서 끝나는 지 x로 찾고
6. 그 다음에 x 부터 읽어서 그냥 n - 2부터 차곡차곡 올리면 됐었고
7. 그리고 x + y >= n 이렇게 했으면 됐는데 그냥 break; 문을 괜히 x + 1 + y > n 이렇게 해서 맞는 건가 아닌 건가 헷갈리고
8. 그리고 spread에서 0인 애들이랑도 물고기 교환하고 , 이러한 실수들이 있었음
9. 실수가 엄청 많은 문제였는데 그래도 진짜 막막했는데 다시 생각하고 다시 처음부터 짜서 괜찮았음
10. arrange만 빼면 설계 자체는 그대로 진행하였음
 */
public class Main {
