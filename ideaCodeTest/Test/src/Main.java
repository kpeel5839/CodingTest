...
public class Main {
    public static void main(String[] args) {
        System.out.println("hw1_2 : 김자경");

        int n = 10;
        int startTime = 1;
        int endTime = 6;

        //(1) 원소 수가 10인 Reservation형 배열 생성
        ...
        for(int i=0; i<array.length; i++) {
            startTime = random.nextInt(5)+1;

            endTime = startTime + 1 + random.nextInt(6 - startTime);     //2~6시어야하고 시작시간보다 최소 1시간 이상 커야 함..
        ...
        //(4) 각 길이의 예약이 몇 개 씩인지 구함 (중첩 반복문을 사용하지 말고 수행시간 복잡도가 O(n)이 되도록 작성해 볼 것)
        int [] count = new int[5];
        for(int i=0; i<array.length; i++) {
